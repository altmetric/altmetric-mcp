...
export function describeError(error) {
  if (!(error instanceof Error)) {
    ...
    return `type=unknown message="${redact(text)}" message_sha256_prefix=${sha256Prefix(text)}`;
  }

  const message = typeof error.message === 'string' ? error.message : '';
  return `type=${error.name} message="${redact(message)}" message_sha256_prefix=${sha256Prefix(message)}`;
}
