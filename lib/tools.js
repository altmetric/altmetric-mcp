...
// That endpoint also rejects a date range it cannot serve, and does so with an
// unhandled error rather than a JSON error body, so catch both cases here and
// say what to change.
const MAX_MENTION_SOURCES_RANGE_MS = 366 * 24 * 60 * 60 * 1000;
...
function assertMentionSourcesDateRange({ mentioned_after: after, mentioned_before: before }) {
  ...
  if (Date.parse(before) - Date.parse(after) > MAX_MENTION_SOURCES_RANGE_MS) {
    throw new Error('explore_mention_sources accepts a date range of at most one year; narrow mentioned_after/mentioned_before');
  }
}
