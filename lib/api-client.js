...
import { sanitizeUpstreamText } from './output-guard.js';
import { sha256Prefix } from './log-redact.js';
...
function assertHttps(url) {
  ...
  // Allow plaintext http only for loopback, so the HTTP transport can run against a
  // local Explorer in dev/test. Remote upstreams must always be https — credentials
  // travel on these requests.
...
export function generateExplorerDigest(filters, secret) {
  ...
  // The Explorer API verifies the digest using the secret with its dashes removed, so a
  // UUID-format secret must be normalised the same way here, otherwise the signature is
  // rejected as invalid. Stripping is a no-op for a dash-free secret.
  const hmacSecret = secret.replace(/-/g, '');
...
export async function makeDetailsApiRequest(endpoint, params = {}, apiKey, baseUrl, fetchOptions = {}) {
  ...
  if (!response.ok) {
    ...
    throw new Error(upstreamErrorMessage(response.status, errorText));
  }
...
// Build the user-facing message for a failed upstream request.
//
// Altmetric's APIs return structured JSON:API errors (status/code/title/detail) that are
// intended for API consumers, so when the body is one of those we surface its title/detail/code
// (sanitised) — turning an opaque "HTTP 400" into an actionable reason (e.g. an organisation
// lacking Explorer API access). For anything else we fall back to a generic status message.
// We never surface the raw body: it can echo internal hostnames, IPs, or our API key, and only
// the curated structured fields are safe to show.
function upstreamErrorMessage(status, body) {
  if (typeof body === 'string' && body.length > 0 && body.length <= 64 * 1024) {
    try {
      ...
      const message = [err?.title, err?.detail].filter(Boolean).join(': ');
      if (message) {
        const withCode = err.code ? `${message} (${err.code})` : message;
        // Sanitise the first-party error text through the prompt-injection guard before it
        // reaches the LLM; the trailing status is ours and safe.
        return `${sanitizeUpstreamText(withCode, 400)} [HTTP ${status}]`;
      }
    } catch {
      // Not a structured JSON:API error — fall through to the generic message.
    }
  }
