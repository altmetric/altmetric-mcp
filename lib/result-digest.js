...
// Every field lands on a single line of a list the model reads, so collapse
// whitespace before anything else: a newline inside an upstream title or
// account name would otherwise split one item across two lines, letting that
// text pose as a further result or as one of our own truncation notes.
function clean(value) {
  if (value === null || value === undefined || value === '') return null;
  const collapsed = String(value).replace(/\s+/g, ' ').trim();
  if (collapsed === '') return null;
  const text = sanitizeUpstreamText(collapsed, MAX_FIELD_LEN);
  return text === '' ? null : text;
}

// id -> display name for anything the response embedded, so a mention can name
// its author instead of printing an opaque profile id. The block arrives as an
// array on the record endpoints and is documented as a keyed object elsewhere,
// so read either: on the object shape the id can live in the key rather than in
// the entry.
function namesFromIncluded(included) {
  const names = new Map();
  if (!included || typeof included !== 'object') return names;

  const entries = Array.isArray(included)
    ? included.map((entry) => [entry?.id, entry])
    : Object.entries(included);

  for (const [key, entry] of entries) {
    const name = entry?.attributes?.name ?? entry?.attributes?.title ?? entry?.name ?? entry?.title;
...
function describePosts(posts, held = {}) {
  ...
  for (const [source, entries] of Object.entries(posts)) {
    ...
    const authors = entries
      ...
      .filter(Boolean);
    // The ellipsis means "more posts than are named here", so it follows from
    // the cap, not from how many of the first few happened to be nameable.
    const more = entries.length > MAX_POST_AUTHORS && authors.length > 0 ? ', …' : '';
