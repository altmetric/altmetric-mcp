...
export function createCredentialsBroker({
  ...
  async function get(token) {
    ...
    // Sweep entries whose tokens expired and were never looked up again, so the cache
    // can't grow unbounded as OAuth tokens rotate on the long-lived HTTP server.
    const t = now();
