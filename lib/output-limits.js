...
function trimPostsPerSource(result, sc, maxBytes, refreshDigest) {
  ...
  const apply = (cap) => {
    ...
    // The digest grows as this shrinks - a held-back source renders as
    // "news (20 of 300)" rather than "news (300)" - so it has to be rebuilt
    // before the size is measured, or the search fits the payload against a
    // digest it is not going to send.
    refreshDigest();
  };
...
export function enforceResultSizeLimit(result, maxBytes = MAX_RESULT_BYTES) {
  ...
  if (!trimmed && byteLength(result) > maxBytes) {
    const cap = trimPostsPerSource(result, sc, maxBytes, refreshDigest);
    if (cap >= 1) {
