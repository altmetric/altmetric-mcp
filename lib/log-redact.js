...
export function describeError(error) {
  if (!(error instanceof Error)) {
    ...
    return `type=unknown message="${redact(text)}" message_sha256_prefix=${digest(text)}`;
  }

  const message = typeof error.message === 'string' ? error.message : '';
  return `type=${error.name} message="${redact(message)}" message_sha256_prefix=${digest(message)}`;
}
