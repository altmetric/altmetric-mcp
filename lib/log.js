...
// Short SHA-256 prefix, the form every field here uses to stand in for content it must
// not print.
...
/**
 * The redacted message on its own, for the other place an error is rendered: the text this
 * server hands back to the client. In stdio mode that text lands in the same host transcript
 * as the log line, so redacting one and not the other would leave the content in the
 * transcript anyway.
 *
 * @param {unknown} error
 * @returns {string}
 */
export function describeErrorForClient(error) {
  return truncate(redactedMessageOf(error));
}
...
/**
 * Record a handled failure. This is the only way this server writes an error to stderr, so
 * that "a stderr line must not carry caller data" is a property of the call rather than
 * something each call site has to remember. The label is forced onto one line so a record
...
export function logError(label, error) {
  console.error(`${String(label).replace(/[\r\n]+/g, ' ')}: ${describeError(error)}`);
}

/**
 * Record a failure that happened before there were any requests: transport setup, binding a
 * socket. Deliberately keeps the whole error, stack included - nothing a caller sent can be
 * in scope yet, and the stack is the useful artifact of a boot failure. The exception is
 * named rather than left to an unwrapped console.error, so it reads as a decision.
 *
 * @param {string} label
 * @param {unknown} error
 */
export function logFatal(label, error) {
  console.error(label, error);
}

/**
 * Record a non-2xx from an upstream API. The body is digested rather than printed: bodies
 * can echo internal hostnames, IPs, or even our own API key in some error paths.
 *
 * @param {string} label
 * @param {number} status
 * @param {string} body
 */
export function logUpstreamError(label, status, body) {
